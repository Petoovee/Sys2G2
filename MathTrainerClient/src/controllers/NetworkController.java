...
/**
 * Class Network handles the outgoing and incoming information to and from the
 * server. It connects with a socket, sets up streams and has two inner classes
 * handling the monitoring for new information to send or recieve.
 * 
...
public class NetworkController {
	...
	/**
	 * Creates the socket that connects to the server, gets buffers (from
	 * MainController) and starts the threads for communicating with the server.
	 * 
	 ...
	 * 
	 * @param request The request. Contains information of the type of request, and
	 *                relevant data
	...
	/**
	 * Receives the incoming messages from the server and adds them to the incoming
	 * buffer.
	 * 
	 * @author Niklas Hultin, Bajram Gerbeshi
	 */
	private class NetworkHandler {
		...
		/**
		 * Handles a the splitting of the request and the data for a String-only
		 * request.
		 * 
		 * @param request Contains both the request type and the data, separated by a
		 *                blank space.
		 * @author Bajram Gerbeshi
		 */
		...
		/**
		 * Sends a request to the server and gets an object in return in a sequential
		 * manner.
		 * 
		 * @return The object returned by the server. This is either a User or a String
		 *         (handled in MainController)
		 * @author Niklas Hultin, Bajram Gerbeshi
		 */
