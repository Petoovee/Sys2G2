...
/**
 * Class MainController is controller of the other controllers. All scene
 * controllers has a reference to this controller to communicate with it.
 * NetworkController communicates with it through the buffers. In this class,
 * only the general and recurring logic is handled, such as setting scenes
 * (inner class), pop-up windows and closing the program.
 * 
...
public class MainController {
	...
	/**
	 * Starts the network that connects to the server and creates and populates the
	 * ScenesHashMap. It also sets up the window with the correct information for
	 * the start screen.
	 * 
	 * @param mainWindow The main window where the scenes are displayed (received
	 *                   from Main)
	...
	/**
	 * Uses the interface SceneControllerParent (used by all Scene-controllers) to
	 * send itself to the scene-controller for the FXML-file loaded in to the
	 * FXMLLoader. This gives the Scene-controllers a reference to this
	 * MainController.
	 * 
	 * @param loader The loader for a specific FXML-file. This gives access to that
	 *               files specified controller.
	 * @author Niklas Hultin
	 */
	...
	/**
	 * Uses a static class Alarm to create Pop-Up boxes. The box will be the only
	 * interactive object until it has been handled by the user. The code is only
	 * made for using the AlertTypes Confirm (Yes/no-option) and Error (Only OK).
	 * 
	 * @param alertType The type of box. Confirm gives a box with OK and Cancel
	 *                  buttons. Error only gives an OK button.
	 * @param title     The title of the pop-up window
	 * @param message   The message shown in the pop-up window.
	 * @return Returns true if the user pressed the OK button, otherwise returns
	 *         false. The return value is not always useful (the Error Pop-Up only
	 *         has one option, for example) and it is up to the developer to handle
	 *         it correctly.
	 * @author Niklas Hultin
	 */
	...
	/**
	 * This method is used both when the user hits the button to close to program,
	 * and if the user tries to shut down the program using the standard close
	 * option in the OS (for example: X in the top right corner of the window).
	 * 
	 * @author Niklas Hultin
	 */
	...
	/**
	 * Method is used when creating a new user. It is sent to the server and
	 * depending on the result, the new user is either logged in or an error message
	 * is shown.
	 * 
	 * @param username
	 * @param password
	...
	public void newUser(String username, String password, String city, String school, Object year) {
		...
		if (returnValue instanceof User) {
			...
			popUpWindow(Alert.AlertType.ERROR, errorString.substring(0, errorString.indexOf(':')),
					errorString.substring(errorString.indexOf(':') + 2));
		}
	}
	...
	/**
	 * Method is used to pass object of questions to network Controller and handle
	 * the return value.
	 * 
	...
	/**
	 * Used to update the results, if needed, stored in the user object and send
	...
	/**
	 * Generic way of setting the initial values of a scene when it is being shown.
	 * Each Scene's controller knows how to handle the type of object that they will
	 * recieve.
	 * 
	 * @param object
	 * @author Niklas Hultin
	...
	/**
	 * Logs the user out (as no constant connection is held with the server, the
	 * server is not informed). The change of scene and currentUser ensures that the
	 * same user cannot make any changes without logging in again.
	 * 
	 * @author Niklas Hultin
	 */
	public void logOut() {
	...
	/**
	 * Changes to the home scene without logging in. This means no user information
	 * is being stored.
	 * 
	 * @author Niklas Hultin
	 */
	...
	/**
	 * Changes to the game start-up scene. The initial values of the scene varies
	 * depend if currentUser is null or not.
	 * 
	 * @author Niklas Hultin
	 */
	...
	/**
	 * Inner class SceneSetter handles the Scenes. It loads them, hands over the
	 * controllers to the MainController for communication, and handles
	 * communication with the ScenesHashmap. It also sets up the current scene.
	 * 
	 * @author Niklas Hultin
	 * @version 1.0
	 */
	private class SceneSetter {
		...
		/**
		 * Loads the scenes and adds every fxml-scene to the HashMap with the
		 * corresponding ScenesEnum-name.
		 * 
		 * @throws IOException
		 */
		private void addScenesToHashMap() throws IOException {
			...
			FXMLLoader quizCompletedLoader = new FXMLLoader(
					getClass().getResource("../scenes/mainMenu/QuizCompleted.fxml"));
